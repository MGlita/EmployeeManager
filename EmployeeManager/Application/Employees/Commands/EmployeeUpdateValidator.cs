...
namespace Application.Employees.Commands
{
    public class EmployeeUpdateValidator : AbstractValidator<UpdateEmployee>
    {
        public EmployeeUpdateValidator()
        {
            ...
            RuleFor(x => x.Employee.BirthDate).NotNull().GreaterThan(DateTime.Now.AddYears(-18).AddHours(1));
            RuleFor(x => x.Employee.Email).NotNull().MaximumLength(50).EmailAddress();
            RuleFor(x => x.Employee.HiredDate).NotNull().GreaterThan(DateTime.Now.AddDays(1));
